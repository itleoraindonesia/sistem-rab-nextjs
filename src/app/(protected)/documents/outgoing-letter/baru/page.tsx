...
import { useDocumentTypes, useInstansiList, useUsersList, useCreateLetter, useUpdateLetter, useSubmitForReview } from "../../../../../hooks/useLetters"
...
export default function BuatSuratKeluarPage() {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      // Redirect immediately to detail page tailored to the created letter
      router.push(`/documents/outgoing-letter/${letter.id}`)
    } catch (err: any) {
      setError(err.message || 'Gagal menyimpan draft')
      setLoading(false) // moved from finally to ensure loading state is cleared on error
    } 
    // finally block removed as redirect happens on success
  }
