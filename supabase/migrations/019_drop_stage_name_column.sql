-- Migration: Drop stage_name column and update functions to use stage_type
-- Date: 2026-03-05
-- Description: Remove stage_name column as it's redundant with stage_type

-- =====================================================
-- PART 1: Update functions to use stage_type instead of stage_name
-- =====================================================

-- Update submit_letter_for_review function
...
BEGIN
  ...
  SELECT
    ...
    'REVIEW',
    ws.sequence,
    'Review task assigned - Stage: ' || ws.stage_type,
    now()
...
-- Update resubmit_revision function
DROP FUNCTION IF EXISTS resubmit_revision(UUID, UUID);
...
BEGIN
  ...
  SELECT
    ...
    'REVIEW',
    ws.sequence,
    'Review task reassigned - Stage: ' || ws.stage_type,
...
COMMENT ON FUNCTION submit_letter_for_review(UUID, UUID) IS
  'v019: Submit draft letter untuk review. Uses stage_type instead of stage_name.';

COMMENT ON FUNCTION resubmit_revision(UUID, UUID) IS
  'v019: Resubmit surat setelah revisi. Uses stage_type instead of stage_name.';

-- =====================================================
-- PART 2: Update existing data in letter_history if needed
-- =====================================================

-- No data migration needed since we're just changing column structure
-- The historical messages with stage_name will remain as-is

-- =====================================================
-- PART 3: Drop the stage_name column
-- =====================================================

-- Make column nullable first (safer approach)
...
COMMENT ON COLUMN document_workflow_stages.stage_type IS 'Type of workflow stage: REVIEW or APPROVAL. This replaces the deprecated stage_name column';
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'Warning: stage_name column still exists';
  ELSE
    RAISE NOTICE 'Success: stage_name column has been dropped';
  END IF;
END $$;
